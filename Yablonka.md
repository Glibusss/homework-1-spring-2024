...
* При уменьшении окна по вертикали до:
    - 576px, пропадают изображения рядом с полями ввода
* При уменьшении окна по горизонтали до:  
    ...
    - 575px, пропадают изображения рядом с полями ввода, уменьшаются растояния между элементами меню и их размеры
* В мобильной версии отсутствуют изображения рядом с полями ввода
* Некорректный email - это текст, не имеющий знак @.
* Некорректный пароль - это пароль, не соответствующий условию "Пароль должен содержать лишь латинские буквы, цифры, спецсимволы и быть длиннее 8 символов"

## Меню входа

![Меню входа](./images/login.png)
* При нажатии на кнопку "Войти" или на любое поле ввода с последующим нажатием клавиши Enter происходит отправка данных входа
* При нажатии на надпись "Регистрация" происходит переход на [страницу регистрации](https://nota-tabula.ru/signup) 
...
* При вводе пароля, некорректного пароля, отображается надпись "Пароль должен содержать лишь латинские буквы, цифры, спецсимволы и быть длиннее 8 символов"
* При вводе верных логина и пароля, пользователя перенаправляет на [страницу досок](https://nota-tabula.ru/main)
...
* При уменьшении окна по вертикали до:
    - 769px, пропадают изображения рядом с полями ввода
...
* При уменьшении окна по горизонтали до:  
    ...
    - 575px, пропадают изображения рядом с полями ввода, уменьшаются растояния между элементами меню и их размеры, пропадает текст "Регистрация"
...
* Для перехода на [страницу входа](https://nota-tabula.ru/signin), нужно нажать на текст "Уже есть аккаунт?"
* При вводе некорректного email, отображается надпись "Некорректный email"
...
* При вводе некорректного пароля, отображается надпись "Пароль должен содержать лишь латинские буквы, цифры, спецсимволы и быть длиннее 8 символов", даже если поле подтверждения пусто
* При несоответствии паролей в двух полях ввода, выводится сообщение "Пароли не совпадают"
* При вводе верных данных для несуществующего аккаунта, пользователя перенаправляет на [страницу досок](https://nota-tabula.ru/main)

# [Хедер](https://nota-tabula.ru/main)
...
* В случае если поле "Название доски" заполнено некорректно (содержит один или более спецсимволов "&|`<>/()+=.,~№{}[]", не входящих в допустимые) под полем отображается предупреждение о допустимых символах
...
* Некорректное имя - имя, не соответствующее условию "Имя может содержать лишь буквы кириллицы и латиницы, цифры, спецсимволы $@!?#^:;%'"\*_ и быть не пустым"
* Некорректная фамилия - фамилия, не соответствующая условию "Фамилия может содержать лишь буквы кириллицы и латиницы, цифры, спецсимволы $@!?#^:;%'"\*_ и быть не пустой"
...
* При переходе авторизованным пользователем на существующую доску, в которую он добавлен, пользователь перенаправляется на [страницу доски](https://nota-tabula.ru/workspace/28/board/84)
...
* При нажатии в левом боковом меню нажать на кнопку с плюсом справа от надписи "Участники" отображается окно "Добавить пользователя на доску" с полем ввода email
    * Если поле ввода пустое, то кнопка "Добавить" становится бледной, при клике на неё ничего не происходит. Кнопка "Удалить" не изменяется визуально, но при клике на неё также ничего не происходит
    * При вводе некорректного email отображается надпись "Некорректный email"
...
* При клике на кнопку "Добавить список" открывается чёрное окно с полем ввода названия списка, кнопкой "Добавить список" и крестиком
    * При нажатии на крестик окно закрывается
    * При вводе некорректного названия отображается ошибка "Некорректное название"
    * В случае если поле ввода пустое, кнопка "Добавить список" становится бледной. При клике на неё ничего не происходит
...
* При нажатии на надпись "Добавить карточку" в любом списке карточек, появляется окно с полем ввода названия карточки и кнопками "Добавить карточку" и крестиком.
    * При нажатии на крестик окно закрывается
    * При вводе некорректного названия отображается ошибка "Некорректное название"
    * В случае если поле ввода пустое, кнопка "Добавить карточку" становится бледной. При клике на неё ничего не происходит
...
* При нажатии на кнопку "Даты" открывается модальное окно для добавления дат карточки.
    ...
    * В поле дат необходимо вручную ввести нужную дату или использовать нативный элемент календаря, нажав на соответствующую кнопку календаря справа в поле ввода.
...
* В горизонтальном положении мобильной версии, при нажатии на окно ввода, появившаяся клавиатура закрывает собой или отодвигает всплывающие окна, полностью убирая вознеобходимость увидеть поле ввода
